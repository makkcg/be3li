...
class Core {
    ...
    function getUrl() {
	   ...
	   return "http://shop.be3li.com";
	  // return "http://localhost/be3ly/vo/index.php";
    }
	/**function to return the imgs path from bo**/
	function getboimgPathUrl($database_manager){ 
		...
		if($rowxx['URLsubfolder']==""){
			$imagepath=$rowxx['baseUrl'].'/bo/';
		}else{
	...
	/****function to set or unset product to favourits***/
	function add_rem_item_fav($database_manager,$productid,$set1_unset0){
		$irid=$_SESSION['ir_id'];
		/***check if the product fav is in db***/
		$sql="SELECT `id`,`irid`,`productid`,`isfav` FROM `fav_items` WHERE `productid`='".$productid."' AND `irid`='".$irid."'";
		$result = $database_manager->query($sql);
		if($result){
			$row = mysqli_fetch_assoc($result);
			/****update the record with the new fav status***/
			$sql1="UPDATE  `fav_items` SET  `isfav` =  '".$set1_unset0."' WHERE  `productid`='".$productid."' AND `irid`='".$irid."'";
			
		}else{
			/***else insert the new fav record***/
			$sql1="INSERT INTO `fav_items` (`id`, `irid`, `productid`, `isfav`) VALUES (NULL, '".$irid."', '".$productid."', '".$set1_unset0."');";
		}
	...
	private function isuserverified(){
		return  $_SESSION['isactivated'] && isset($_SESSION['isactivated']);
	}
	function verifyiraccount($redirect) {

        if (!$this->isuserverified()) {
            header("Location: " . $this->getURL() . "?page=verifyirdata&redirect=" . $redirect);
        }
    }
...
/***************************Recharge Card functions****************/
	...
	function getewalletval($database_manager,$irid){
		 $sql = "SELECT ewallet FROM ir "
                . " WHERE ir_id = '" . $irid . "'";
        $result = $database_manager->query($sql);
        $row = mysqli_fetch_assoc($result);
