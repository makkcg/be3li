 ...
 * @version 1.5.2 build 20150215-2219
...
class jqgrid
{
				...
				foreach($rules as $key=>$val) 
				{
					...
					foreach($this->options["colModel"] as $link_c)
					{
						...
						if ($val['field'] == $link_c["name"] && !empty($link_c["formatoptions"]) && in_array($op, array("ne","eq","gt","ge","lt","le")))
						{
							...
							if (strstr($link_c["formatoptions"]["newformat"],"d-m-y"))
							{
					...
					if(isset($v) && isset($op))
					{
						...
						switch ($op) {
							...
							case 'cn' :
								...
								if (strpos($this->db_driver,"oci8") !== false)
	...
	function set_columns($cols = null, $change_field = false)
	{
		...
		for($i=0;$i<count($cols);$i++)
		{
			...
			if (isset($cols[$i]["formatter"]) && $cols[$i]["formatter"] == "date")
			{
				...
				// only used exact date match, when operator is not 'cn' (contains) - default is cn
				if ( in_array($cols[$i]["searchoptions"]["sopt"], array("ne","eq","gt","ge","lt","le")) )
				{
			...
			if (isset($cols[$i]["formatter"]) && $cols[$i]["formatter"] == "autocomplete")
			{
				...
				$cols[$i]["editoptions"]["dataInit"] = "function(o){link_autocomplete(o,$param);}";
			}
	...
	function render($grid_id)
	{
		...
		if (count($_FILES) && ($_REQUEST["grid_id"] == $grid_id || "_".$_REQUEST["subgrid"] == $grid_id))
		{
			...
			if(!empty($_FILES[$fileElementName]['error']))
			{
				...
				{
					case '1':
						$error = $_SESSION['main_language']->uploaded_file_is_too_big ;
						break;
					case '2':
				...
				case "autocomplete":
					...
					foreach( $this->options["colModel"] as $c )
					{
						if ($c["index"] == $field)
						{
							...
							$sql = $c["formatoptions"]["sql"]. " $cond {$search_on} like '%$term%'";
							$result = $this->execute_query($sql);
							if ($this->con)
							{
		...
		if (isset($_GET['jqgrid_page']))
		{
			...
			// use tilda sign for sort by + dont add where multiple sort e.g. fix `gender asc, name` desc
			if (strpos($this->db_driver, "mysql")!== false || !isset($this->db_driver))
				if($sidx != 1 && strstr($sidx,",") === false)
					$sidx = "`" . $sidx . "`";
				...
				foreach($this->options["colModel"] as $c)
				{
					foreach ($arr as &$rec)
					{
						...
						if (!empty($rec[$c["name"]]) && ($c["formatter"] == "date" || $c["formatter"] == "datetime"))
						{
							...
							if ($dt != "1970-01-01" && $dt != "0000-00-00")
			...
			foreach ($rows as $row)	
			{
				...
				foreach($this->options["colModel"] as $c)
				{
					...
					if (!empty($row[$c["name"]]) && ($c["formatter"] == "datetime") )
					{
						...
						$row[$c["name"]] = date($js_dt_fmt,strtotime($dt));
					}
		...
		{
			$this->options["edit_options"]["afterShowForm"] = "function(){ return fx_bulk_unrequire('{$grid_id}'); }";
		}
				...
				if (isset($this->options["detail_grid_id"]) && $this->options["detail_grid_id"] != '') 
				{
					...
					foreach($d_grids as $detail_grid_id) 
					{
						...
						if(ids == null) 
						{
							...
							{
								jQuery('#".$detail_grid_id."').jqGrid('setGridParam',{url:'".$detail_url."&rowid='+ids+data,editurl:'".$detail_url."&rowid='+ids+data,jqgrid_page:1});
				...
				$out .= "if(ids && ids.rows) jQuery.each(ids.rows,function(i){
						...
						if (count($this->conditional_css))
						{
							foreach ($this->conditional_css as $value) 
							{
								...
								$out .= "if (!jQuery.isNumeric(this.{$value[column]})) this.{$value[column]} = this.{$value[column]}.replace(/(<([^>]+)>)/ig,'');";							
								if ($value["op"] == "cn")
								{
									$out .= "
	...
	?>
		...
		function fx_bulk_unrequire(list)
		{
			...
			for (var a=0;is_bulkedit && a < checkbox_fields.length;a++)
			{
				...
				str = '<select class="FormElement ui-widget-content ui-corner-all" onchange="$(\'#'+checkbox_fields[a]+'\').val(this.value);" name="'+checkbox_fields[a]+'"><option value="'+vals[0]+'">Checked</option><option value="'+vals[1]+'">Un-Checked</option> </select>';
		...
		jQuery(document).ready(function(){
			jQuery(window).bind("resize", function () {
				...
				jQuery(".ui-jqgrid").each(function(i,o){
					...
					var newWidth = jQuery(".ui-tabs:first").width() - 50;
	...
	function remove_subsql()
	{
		...
		// extract subqueries
		preg_match_all("/\(([\s]+select [^)]*)\)/i", $this->select_command, $subsql);	
		$subsql = $subsql[0];
