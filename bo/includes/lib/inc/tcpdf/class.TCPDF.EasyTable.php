...
class TCPDF_EasyTable extends TCPDF {
    ...
    private function _ProcessTableHeader($PageAdded = false) {
        ...
        if ($this->_HeaderCellsFillColor) {
            $this->SetFillColor(
                    $this->_HeaderCellsFillColor['R'], $this->_HeaderCellsFillColor['G'], $this->_HeaderCellsFillColor['B']
            );
        }

        if ($this->_HeaderCellsFontColor) {
            $this->SetTextColor(
                    $this->_HeaderCellsFontColor['R'], $this->_HeaderCellsFontColor['G'], $this->_HeaderCellsFontColor['B']
            );
        }
    ...
    private function _ProcessMultiCellImage($pdfImage, $cellWidth, $cellHeight) {
        ...
        if (!$this->_FillImageCell)
            $this->_FillCell = 0; // no fill
